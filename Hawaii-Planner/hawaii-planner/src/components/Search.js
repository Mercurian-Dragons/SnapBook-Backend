...
const Search = ({searchParams}) => {
    ...
    const [searchFeature, setSearchCity] = useState('')
    const searchOptions ={
    ...
    function getPlaces(searchFeature){
        ...
        fetch(`${searchOptions.geoApi}/geoname?apikey=${searchOptions.key}&name=waikiki`)
        .then(responsed => responsed.json())
        .then(responsed => {
            // console.log(responsed.lon)
            fetch(`${searchOptions.geoApi}radius?radius=100000&lon=${responsed.lon}&lat=${responsed.lat}&kinds=tourist_facilities%2Cinteresting_places&rate=3&apikey=${searchOptions.key}`)
